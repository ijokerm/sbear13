...
//根据中序和后序遍历写出二叉树
   public static TreeNode buildTree2(List<Character>inorder,List<Character>postorder){
    ...
    char rootvalue=postorder.get(postorder.size());
    int leftcount=inorder.indexOf(rootvalue);
...
// 右
       List<Character>rightInorder=inorder.subList(leftcount+1,inorder.size());
       List<Character>rightPosorder=postorder.subList(leftcount,postorder.size()-1);
       TreeNode right=buildTree2(rightInorder,rightPosorder);
       root.right = right;
   return root;
   }
...
List<Character>posorder=Arrays.asList('B','D','G','H','F','C','A');
TreeNode root1=buildTree2(inorder,posorder);
